...
export const getPiezaSelected = (ficha: FichaHexagonal<Color>, x: number, y: number): PiezaTriangular<Color> => {
    if (ficha.numberOfPiezas === 1) return ficha.piezas[0];
    else if(ficha.numberOfPiezas === 2 && !ficha.hasSpaces){
    ...
    }else if(ficha.numberOfPiezas === 3 && !ficha.hasSpaces){
        switch(ficha.rotationStage){
            case 1:
    ...
    }else if(ficha.numberOfPiezas === 4){
        switch(ficha.rotationStage){
            case 1:
    ...
    }else if(ficha.numberOfPiezas === 5){
        switch(ficha.rotationStage){
            case 1:
        ...
        switch(ficha.rotationStage){
            case 1:
                if(x <= 50) return y > -2 * x + 93 ? ficha.piezas[1] : ficha.piezas[0];
                return y < 2 * x - 103 ? ficha.piezas[2] : ficha.piezas[1];
            case 2:
                if(y <= 48) return ficha.piezas[0];
                return y < -2 * x + 172 ? ficha.piezas[1] : ficha.piezas[2];
            case 3:
                if(y > 73.5) return ficha.piezas[2];
                return y < 2 * x - 54.8 ? ficha.piezas[0] : ficha.piezas[1];
            case 4:
                if(x <= 50) return y > 2 * x - 48 ? ficha.piezas[2] : ficha.piezas[1];
                return y < -1.88 * x + 141.5 ? ficha.piezas[1] : ficha.piezas[0];
            case 5:
                if(y >= 73) return ficha.piezas[0];
                return y < -2 * x + 119 ? ficha.piezas[2] : ficha.piezas[1];
            case 6:
                if(y <= 47.5) return ficha.piezas[2];
                return y < 2 * x + 3 ? ficha.piezas[1] : ficha.piezas[0];
        }
    }else if(ficha.numberOfPiezas === 3 && ficha.hasSpaces){
        switch(ficha.rotationStage){
            case 1:
                if(y <= 50){
                    if(x <= 50)return y < 1.895 * x - 46.3 ? ficha.piezas[1] : ficha.piezas[0];
                    return y < -2 * x + 147 ? ficha.piezas[1] : ficha.piezas[2];
                }else{
                    if(x <= 50) return y < -1.92 * x + 141.69 ? ficha.piezas[5] : ficha.piezas[4];
                    return y < 2 * x - 52 ? ficha.piezas[3] : ficha.piezas[4];
                }
            case 2:
                if(y <= 47){
                    if(x <= 50)return y < 1.9375 * x - 42.6 ? ficha.piezas[0] : ficha.piezas[5];
                    return y < -1.93 * x + 144.33 ? ficha.piezas[0] : ficha.piezas[1];
                }else{
                    if(x <= 50)return y < -2 * x + 145 ? ficha.piezas[4] : ficha.piezas[3];
                    return y < 2 * x - 52 ? ficha.piezas[2] : ficha.piezas[3];
                }
        }
    }
